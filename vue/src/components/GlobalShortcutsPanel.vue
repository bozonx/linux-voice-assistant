<template>
  <div class="panel flex flex-row">
    <div class="flex-1 flex flex-row gap-4">
      <Button v-if="menuModalsStore.anyModalOpen" small secondary @click="menuModalsStore.back()">назад (Esc)</Button>
      <Button v-if="isEditorOpen()" small secondary @click="goToEditor">в редактор (Tab)</Button>
...
const keyUpHanlderIndex = globalEvents.addListener(GlobalEvents.KEY_UP, handleShortCutKeyUp);
...
function handleShortCutKeyUp(event: KeyboardEvent) {
  if (event.code === "Escape") {
    if (!menuModalsStore.anyModalOpen) return;
